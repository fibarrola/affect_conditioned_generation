...
for k, img_name in enumerate(img_names):
    ...
    print(
        "{}, E {:.2f}, P {:.2f}, A {:.2f}".format(
            img_name, affs[k, 0].item(), affs[k, 1].item(), affs[k, 2].item()
        )
    )
...
for k, aff_name in enumerate(['E', 'P', 'A']):
    ...
    print(
        f'Max {aff_name}: {img_names[inds[0]]}, {img_names[inds[1]]}, {img_names[inds[2]]}'
    )
    vals, inds = N_max_elements(2 - affs[:, k], 3)
    print(
        f'Min {aff_name}: {img_names[inds[0]]}, {img_names[inds[1]]}, {img_names[inds[2]]}'
    )
