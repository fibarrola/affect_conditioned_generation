...
df = df.dropna()
new_names = {
    col_name: f"{col_name[:2]}_{'most' if k%2==1 else 'least'}"
    for k, col_name in enumerate(df.columns)
    if col_name[4:8] == "Pick"
}
df = df.rename(columns=new_names)
df = df.rename(columns={df.columns[4]: "computer/phone"})
new_names = {
    col_name: f"Q{col_name[:2]}" for col_name in df.columns if col_name[4:7] == "How"
}
df = df.rename(columns=new_names)
...
for k in range(24):
    for choice_of in ["most", "least"]:
        ...
        affect_dim = (
            "Valence"
            if (k // 8) % 3 == 0
            else ("Arousal" if (k // 8) % 3 == 1 else "Dominance")
        )
        ...
        for n in range(1, len(df)):
            if df[col_name].iloc[n] == right_answer:
                ...
                correctness = (
                    ERRORS[2]
                    if df[col_name].iloc[n]
                    == df[
                        "{number:02d}_{choice_of:}".format(
                            number=k + 1, choice_of=other_choice
                        )
                    ].iloc[0]
                    else ERRORS[1]
                )
...
color_dict = {
    "VQGAN+CLIP": {"Valence": "#a02c2c", "Arousal": "#5aa02c", "Dominance": "#2c5aa0"},
    "StableDifussion": {
        "Valence": "#d35f5f",
        "Arousal": "#8dd35f",
        "Dominance": "#5f8dd3",
...
patterns = {"VQGAN+CLIP": "", "StableDifussion": "/"}
fig = go.Figure()
for aff_dim in ["Valence", "Arousal", "Dominance"]:
    for generator in ["VQGAN+CLIP", "StableDifussion"]:
        aux = sorting_data[sorting_data["affect_dim"] == aff_dim]
        ...
        xx = ERRORS
        yy = [
            len(aux[aux["correctness"] == correctness]) / len(aux) for correctness in xx
        ]
        ...
        fig.add_trace(
            go.Bar(
                x=xx,
                y=yy,
                name=f"{generator}, {aff_dim}",
                marker={"color": color_dict[aff_dim]},
                marker_pattern_shape=patterns[generator],
            )
        )
fig.update_layout(
    title={"text": "Correct Identification Rates"},
...
for col_name in col_names:
    if col_name[:8] == 'How well':
        affect_cond = (
            "No affect" if refs[col_name].item() == 1 else "Affect-conditioning"
        )
        for x in df_old[col_name]:
            if not np.isnan(x):
                quality_data.append(
                    {"score": x, "conditioning": affect_cond, "generator": "VQGAN+CLIP"}
                )
...
for col_name in df.columns:
    if col_name[0] == "Q":
        affect_cond = (
            "Affect-conditioning" if df[col_name].iloc[0] == 7 else "No affect"
        )
        for n in range(1, len(df)):
            quality_data.append(
                {
...
if SHOW_RATINGS:
    ...
    for generator in ["VQGAN+CLIP", "StableDifussion"]:
        ...
        yy = [
            len(aux[aux["is correct"] == correctness]) / len(aux) for correctness in xx
        ]
        fig.add_trace(go.Bar(x=xx, y=yy, name=generator))
    fig.update_layout(
        title="Correct Identification Rates",
