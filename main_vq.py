...
from src.vqclip_utils import MakeCutouts, Prompt, parse_prompt, resize_image, vector_quantize, clamp_with_grad, add_stegano_data, add_xmp_data
...
class AffectiveGenerator():
    def __init__(self, vqg_name="vqgan_model/vqgan_imagenet_f16_16384", im_size=[480, 480], cutn=64):
        ...
        self.vqg_model = load_vqgan_model(vqg_name+'.yaml', vqg_name+'.ckpt').to(device)
        self.im_size = im_size
        self.clip_model = clip.load('ViT-B/32', jit=False)[0].eval().requires_grad_(False).to(device)
        self.e_dim = self.vqg_model.quantize.e_dim
        
        cut_size = self.clip_model.visual.input_resolution
        self.make_cutouts = MakeCutouts(cut_size, cutn, cut_pow=1.)
        self.z_min = self.vqg_model.quantize.embedding.weight.min(dim=0).values[None, :, None, None]
        self.z_max = self.vqg_model.quantize.embedding.weight.max(dim=0).values[None, :, None, None]
    ...
    def initialize(self, prompts, v=None, img_0=None, target_imgs=None, seed=None, lr=0.1, noise_prompt_seeds=[], noise_prompt_weights=[], ):
        ...
        for seed, weight in zip(noise_prompt_seeds, noise_prompt_weights):
            ...
            embed = torch.empty([1, self.clip_model.visual.output_dim]).normal_(generator=gen)
            self.pMs.append(Prompt(embed, weight).to(device))

    def synth(self, z):
        z_q = vector_quantize(z.movedim(1, 3), self.vqg_model.quantize.embedding.weight).movedim(3, 1)
    ...
    def ascend_txt(self, init_weight=0):
        global i
        out = self.synth(self.z)
        iii = self.clip_model.encode_image(self.normalize(self.make_cutouts(out))).float()
