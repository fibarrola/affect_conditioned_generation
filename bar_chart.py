...
colors = ["HSL(233,86,70)","HSL(5,67,70)","HSL(131,66,70)",]
descriptions = ["A close-up snake", "An antelope drinking water", "A car crash", "A smiling woman sitting on the beach"]
fig = make_subplots(rows=1, cols=4, subplot_titles=descriptions, shared_yaxes=True,)

for p, prompt in enumerate(descriptions):
    fil_df = df[df["Image Description"]==prompt]
    for n, name in enumerate(["Img. ground truth","Pred. from image","Pred. from prompt"]):
