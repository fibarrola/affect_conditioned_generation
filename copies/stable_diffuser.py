...
class StableDiffuser:
    ...
    def initialize(
        ...
        if start_code is None:
            self.start_code = torch.randn(
                [n_samples, self.C, self.H // self.f, self.W // self.f], device=device,
            )
        ...
        for ps in tqdm(self.data, desc="data"):
            ps = ps
        self.zz = self.model.get_learned_conditioning(ps)
    ...
    @torch.no_grad()
    def run_diffusion(
        self, scale=7.5, ddim_steps=50, ddim_eta=0.0, save=True, suffix=''
    ):
            ...
            with self.model.ema_scope():
                ...
                if save:
                    ...
                    for x_sample in x_checked_image_torch:
                        ...
                        img.save(
                            os.path.join(
                                self.sample_path, f"{suffix}_{self.base_count:05}.png"
                            )
                        )
                        self.base_count += 1
